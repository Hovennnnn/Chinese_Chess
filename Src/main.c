...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	Path path;
	ChangeBuffer changeBuffer; // 记录当前棋子移动的数据

	char assassin; // 刺客，表示将军的棋子
  /* USER CODE END 1 */
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
  NVIC_EnableIRQ(EXTI0_IRQn);  //  启动悔棋按键中断
  NVIC_EnableIRQ(EXTI9_5_IRQn); // 启动外部中断，触摸屏设备通过中断通知用户程序
...
//    BSP_LCD_LayerDefaultInit(LTDC_ACTIVE_LAYER_BACKGROUND, LAYER0_ADDRESS); // 初始化back层
  BSP_LCD_LayerDefaultInit(LTDC_ACTIVE_LAYER_BACKGROUND, (u32*)gImage_board); //直接用gImage_board初始化background
  BSP_LCD_LayerDefaultInit(LTDC_ACTIVE_LAYER_FOREGROUND, LAYER1_ADDRESS); // 初始化front层
  BSP_LCD_DisplayOn();
  ...
  while (1)
  {
	  ...
	  tempPiece = piece[path.indexS];
	  		...
	  		{
	  			change(path.indexS,path.x,path.y,&changeBuffer);//先更改棋子的数据和棋局数据，界面并没有改变

	  			if(check(!tempPiece.camp,&assassin) || generalLine())
	  			{
	  				backChange(&changeBuffer);//这样走不行
	  				deSelectPiece(path.indexS);//取消选中

	  				write_Chinese(sX,sY,'无',LCD_COLOR_DARKBLUE,Libbuttom,lengthCH2);
	  				write_Chinese(sX+lengthCH2,sY,'效',LCD_COLOR_DARKBLUE,Libbuttom,lengthCH2);
	  				clearFlag = 1;
	  			}
	  			else
	  			{
	  				clearPiece(changeBuffer.AX,changeBuffer.AY);//旧棋子擦除
	  				makePiece(path.indexS);              //目的地画新棋子
	  				changeBuffer2 = changeBuffer;     
	  				undoEn = 1;                      

	  				if(check(tempPiece.camp,&assassin))//判断是否将军
	  				{
	  					if(judgeWin(assassin))//判断是否将死
	  					{
	  						BSP_LCD_SetTextColor(LCD_COLOR_RED);
	  						BSP_LCD_DisplayStringAt(sX, sY, "game over!", LEFT_MODE);

	  						if(TURN == BLACK_TURN)
	  							write_Chinese(sX,sY,'黑',LCD_COLOR_BLUE,Libbuttom,lengthCH2);
	  						else
	  							write_Chinese(sX,sY,'红',LCD_COLOR_RED,Libbuttom,lengthCH2);

	  						write_Chinese(sX+lengthCH2,sY,'方',LCD_COLOR_CYAN,Libbuttom,lengthCH2);
	  						write_Chinese(sX+lengthCH2*2,sY,'胜', LCD_COLOR_CYAN,Libbuttom,lengthCH2);

	  						changeBuffer2.indexSrc = -2;
	  						while(1);
	  					}
	  				...
	  				else if(clearFlag)  
	  				{
	  					...
	  					BSP_LCD_FillRect(sX, sY, 2*lengthCH2, lengthCH2);
	  					clearFlag = 0;
	  				}
